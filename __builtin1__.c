...
int my_exit(info_t *info)
{
	...
	if (info->argv[1]) 
	{
		...
		{
			info->status = 2;
			printing_error(info, "Illegal number: ");
			_erputs(info->argv[1]);
			_erputchar('\n');
			return (1);
		}
...
int my_cd(info_t *info)
{
	...
	if (chdir_ret == -1)
	{
		printing_error(info, "can't cd to ");
		_erputs(info->argv[1]), _erputchar('\n');
	}
	else
	{
		_seting_env(info, "OLDPWD", get_env(info, "PWD="));
		_seting_env(info, "PWD", getcwd(buffer, 1024));
	}
	return (0);
}
